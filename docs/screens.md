...
│                                 │
│  권한                            │  섹션
│  [마이크] [위치] [카메라]         │  각 항목: 현재 허용 여부 표시
│  [사진/동영상] [동영상] [알림]    │  탭 → OS 설정으로 이동
└─────────────────────────────────┘
```
...
기기 갤러리에서 사진 또는 동영상을 선택한다. 탭 필터로 유형을 구분하고, 날짜별 그룹/무한 스크롤/다중 선택·삭제를 지원한다.
...
```
┌─────────────────────────────────┐
│ ←   갤러리                    [🗑]│  선택 모드 시: "N개 선택됨" + 삭제 버튼
│  [전체(120)] [사진(95)] [동영상(25)]│  TabBar (미디어 수량 표시)
├─────────────────────────────────┤
│ 오늘                          │  │  날짜 그룹 헤더
│ [썸네일] [썸네일] [썸네일🎬]  │  │  (스크롤바: 우측, 드래그 가능)
│ 2026년 6월 10일                │  │
│ [썸네일] [썸네일🎬] [썸네일]  │  │
│  ...                          │  │
└─────────────────────────────────┘
...
| 기능 | 설명 |
|------|------|
| 탭 미디어 수량 | 전체/사진/동영상 탭에 `(수량)` 비동기 표시 |
| 날짜별 그룹 헤더 | 오늘 / 어제 / `YYYY년 M월 D일` |
| 무한 스크롤 | 80개씩 페이지 로드, 하단 400px 전 다음 페이지 자동 fetch |
| 드래그 스크롤바 | `Scrollbar(interactive: true)` — 우측에서 드래그로 빠른 이동 |
| 다중 선택 | 길게 누르기 → 선택 모드 진입, 탭으로 개별 토글 |
| 삭제 | AppBar 🗑 버튼 → 확인 다이얼로그 → `PhotoManager.editor.deleteWithIds` |

#### 권한 상태별 처리

| 상태 | 화면 |
|------|------|
| `PermissionState.authorized` | 그리드 표시 |
| `PermissionState.limited` (부분 접근) | denied와 동일하게 처리 — "모두 허용" 재요청 |
| 그 외 (denied/restricted) | 안내 화면 + [권한 허용하기] + [설정에서 변경] |

**"권한 허용하기" 동작**: `Permission.photos.request()` + `Permission.videos.request()` 순차 호출 →
Android 13/14에서 limited 상태에도 "모두 허용" 시스템 다이얼로그가 재표시됨.

**`READ_MEDIA_VISUAL_USER_SELECTED` 미선언**: AndroidManifest에서 이 권한을 선언하지 않아야 Android 14에서 partial access 옵션이 표시되지 않음 — 루트 원인 수정.
